...
const STATUS_MAP: Record<string, { label: string; cls: string }> = {
  confirmed: { label: 'Onaylandı',    cls: 'bg-emerald-100 text-emerald-700 border-emerald-200' },
  pending:   { label: 'Beklemede',    cls: 'bg-amber-100 text-amber-700 border-amber-200' },
  cancelled: { label: 'İptal Edildi', cls: 'bg-red-100 text-red-700 border-red-200' },
  completed: { label: 'Tamamlandı',   cls: 'bg-blue-100 text-blue-700 border-blue-200' },
}
...
export default function RezervasyonlarimPage() {
  ...
  const TABS: { key: TabKey; label: string; count: number }[] = [
    ...
    { key: 'cancelled', label: 'İptal',      count: cancelled.length },
  ]
        ...
        <div className="flex gap-1 bg-zinc-100 rounded-xl p-1 mb-8">
          {TABS.map(t => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-bold rounded-lg transition-all ${tab === t.key ? 'bg-white shadow-sm text-zinc-900' : 'text-zinc-400 hover:text-zinc-600'}`}
            >
              {t.label} ({t.count})
            </button>
          ))}
        ...
        {active.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-zinc-200">
            ...
            <h3 className="text-lg font-semibold text-zinc-700 mb-2">
              {tab === 'upcoming' ? 'Yaklaşan rezervasyonunuz yok' : tab === 'past' ? 'Geçmiş rezervasyon bulunamadı' : 'İptal edilen rezervasyon yok'}
            </h3>
            {tab === 'upcoming' && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-bold text-zinc-900 text-base">Rezervasyon Detayı</h2>
            ...
            <div className="space-y-3 mb-6">
              {[
                { label: 'İşletme', value: selected.restaurants?.name ?? '—' },
                { label: 'Tarih', value: new Date(selected.reserved_date + 'T12:00').toLocaleDateString('tr-TR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }) },
                { label: 'Saat', value: selected.reserved_time?.slice(0, 5) ?? '—' },
                { label: 'Kişi', value: `${selected.party_size} kişi` },
                { label: 'Durum', value: STATUS_MAP[selected.status]?.label ?? selected.status },
              ].map(f => (
            ...
            {selected.status === 'pending' && selected.reserved_date >= today && (
              ...
              >
                {cancelling === selected.id ? 'İptal ediliyor...' : 'Rezervasyonu İptal Et'}
              </button>
            )}
          </div>
        </div>
