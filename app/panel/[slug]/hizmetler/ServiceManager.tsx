...
export default function ServiceManager({ services, restaurantId }: { services: Service[]; restaurantId: string }) {
  ...
  const [form, setForm] = useState({ ad: '', fiyat: '', sure_dakika: '' })

  async function save(id?: string) {
    if (!form.ad.trim()) return
    const payload = { ad: form.ad, fiyat: Number(form.fiyat) || 0, sure_dakika: form.sure_dakika ? Number(form.sure_dakika) : null }

    if (id) {
      try {
        await apiCall('PATCH', { table: 'hizmetler', id, payload })
      ...
      try {
        const { data } = await apiCall('POST', { table: 'hizmetler', payload })
        setList(prev => [...prev, data as Service])
        toast.show('Eklendi', 'success')
      } catch { toast.show('Eklenemedi', 'error'); return }
    }
    setEditing(null)
    setForm({ ad: '', fiyat: '', sure_dakika: '' })
  }

  async function remove(id: string) {
    try {
      await apiCall('DELETE', { table: 'hizmetler', id })
  ...
  async function toggleActive(s: Service) {
    ...
    try {
      await apiCall('PATCH', { table: 'hizmetler', id: s.id, payload: { aktif: newVal } })
  ...
  function startEdit(s: Service) {
    setEditing(s.id)
    setForm({ ad: s.ad, fiyat: String(s.fiyat), sure_dakika: s.sure_dakika ? String(s.sure_dakika) : '' })
  }
      ...
      <div className="bg-stone-900 border border-stone-800 rounded-2xl p-5">
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <input
            ...
            onChange={e => setForm(f => ({ ...f, sure_dakika: e.target.value }))}
            placeholder="Süre (dk)"
            ...
            >
              {editing === s.id ? (
                <div className="flex flex-col sm:flex-row gap-3">
                  ...
                  <input type="number" value={form.sure_dakika} onChange={e => setForm(f => ({ ...f, sure_dakika: e.target.value }))} className="w-28 bg-stone-800 border border-stone-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-amber-500" />
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {s.sure_dakika && (
                        <span className="flex items-center gap-1 text-[11px] text-stone-500">
                          <Clock size={11} /> {s.sure_dakika} dk
                        </span>
                      )}
                    </div>
                  </div>
